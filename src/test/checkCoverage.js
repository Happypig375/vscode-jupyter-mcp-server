...
function main() {
    ...
    execFileSync(c8Bin, ['--clean=false', 'node', path.join(root, '.vscode-test', 'broker.test.cjs')], opts);
    const out = execFileSync(c8Bin, ['--clean=false', 'node', path.join(root, '.vscode-test', 'kernelFiles.test.cjs')], opts);
    console.log(out);
